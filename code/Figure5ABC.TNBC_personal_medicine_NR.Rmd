...
surface_genes = read.csv(file = "/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/02.Input/Constants/cell_surface_genes_wgene_and_clinical_uniqueStandardNames.txt", header = T) # 2758 genes
surface_genes = as.character(surface_genes[,1])
...
# (Run for once only!) make data for three patients (seurat objects)
```{r}

fig_dir <- "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/03.Results/Figures/"

### Bassez CohortA (patient: BIOKEY_10)
dataset_name = 'Bassez_2021_NatMed'
file_prefix = "seu_Tissue"
data_dir <- paste0("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/02.Input/", dataset_name, "/CohortA/")
processed_dir <- paste0("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/02.Input/", dataset_name, "/CohortA/")
seurat_obj = readRDS(file = paste0(processed_dir,file_prefix,".rds"))
colnames(seurat_obj@meta.data)
unique(seurat_obj$patient_id)
unique(seurat_obj$cellType)
seurat_Bassez_CohortA_sub = subset(seurat_obj, subset = patient_id %in% c("BIOKEY_10"))
seurat_Bassez_CohortA_sub = subset(seurat_Bassez_CohortA_sub, subset = cellType %in% c("Cancer_cell"))
dim(seurat_Bassez_CohortA_sub) # 25288  3139

# Normalize the data
seurat_Bassez_CohortA_sub <- NormalizeData(seurat_Bassez_CohortA_sub) # log(x/sum(x)*10000+1)
seurat_Bassez_CohortA_sub <- FindVariableFeatures(seurat_Bassez_CohortA_sub)
seurat_Bassez_CohortA_sub <- ScaleData(seurat_Bassez_CohortA_sub) # pbmc <- SCTransform(pbmc, vars.to.regress = "CC.Difference")
seurat_Bassez_CohortA_sub <- RunPCA(seurat_Bassez_CohortA_sub, pc.genes = seurat_Bassez_CohortA_sub@var.genes)
seurat_Bassez_CohortA_sub <- RunUMAP(seurat_Bassez_CohortA_sub, reduction = "pca", dims = 1:10) 
seurat_Bassez_CohortA_sub <- FindNeighbors(seurat_Bassez_CohortA_sub, reduction = "pca", dims = 1:10) 
seurat_Bassez_CohortA_sub <- FindClusters(seurat_Bassez_CohortA_sub, resolution = 0.5) # 

saveRDS(seurat_Bassez_CohortA_sub, file = paste0(processed_dir,"Bassez_CohortA_BIOKEY_10_tumorCells.rds"))



### GSE246613 (Patient11)
dataset_name = 'GSE246613'
file_prefix = "seu_Tissue"
data_dir <- paste0("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/02.Input/", dataset_name, "/")
processed_dir <- paste0("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/02.Input/", dataset_name, "/")
seurat_obj = readRDS(file = paste0(processed_dir,"GSE246613_seu_Tissue.rds"))
colnames(seurat_obj@meta.data)
unique(seurat_obj$PatientID)
unique(seurat_obj$celltype_all)
seurat_GSE246613_sub = subset(seurat_obj, subset = PatientID %in% c("Patient11"))
seurat_GSE246613_sub = subset(seurat_GSE246613_sub, subset = celltype_all %in% c("Tumor"))
dim(seurat_GSE246613_sub) # 36601  1087

# Normalize the data
seurat_GSE246613_sub <- NormalizeData(seurat_GSE246613_sub) # log(x/sum(x)*10000+1)
seurat_GSE246613_sub <- FindVariableFeatures(seurat_GSE246613_sub)
seurat_GSE246613_sub <- ScaleData(seurat_GSE246613_sub) # pbmc <- SCTransform(pbmc, vars.to.regress = "CC.Difference")
seurat_GSE246613_sub <- RunPCA(seurat_GSE246613_sub, pc.genes = seurat_GSE246613_sub@var.genes)
seurat_GSE246613_sub <- RunUMAP(seurat_GSE246613_sub, reduction = "pca", dims = 1:10) 
seurat_GSE246613_sub <- FindNeighbors(seurat_GSE246613_sub, reduction = "pca", dims = 1:10) 
seurat_GSE246613_sub <- FindClusters(seurat_GSE246613_sub, resolution = 0.5) # 

saveRDS(seurat_GSE246613_sub, file = paste0(processed_dir,"GSE246613_Patient11_tumorCells.rds"))

```

# load data for three patients (seurat objects)
```{r}

# /Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/02.Input/GSE246613

dataset_name = 'Bassez_2021_NatMed'
processed_dir <- paste0("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/02.Input/", dataset_name, "/CohortA/")
seurat_Bassez_sub = readRDS(file = paste0(processed_dir,"Bassez_CohortA_BIOKEY_10_tumorCells.rds"))

dataset_name = 'GSE246613'
processed_dir <- paste0("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/02.Input/", dataset_name, "/")
seurat_GSE246613_sub = readRDS(file = paste0(processed_dir,"GSE246613_Patient11_tumorCells.rds"))


```

# Figure 4ABC (top). UMAP for three patients 
```{r}

# seurat_Bassez_sub UMAP_Bassez_CohortA_BIOKEY_10_tumorCells.pdf
# seurat_Stefan_sub UMAP_Stefan_AA_3_tumorCells.pdf
# seurat_GSE246613_sub UMAP_GSE246613_Patient11_tumorCells.pdf

DimPlot(
  seurat_Bassez_sub,
  group.by = "seurat_clusters",  # Do not group by any metadata
...
ggsave(
  filename = paste0(fig_dir, "UMAP_Bassez_CohortA_BIOKEY_10_tumorCells.pdf"), # Change file extension to .pdf
  height = 90, 
  width = 90, 
  units = "mm", # Ensure units are in mm
  device = "pdf" # Specify PDF as the output device
)
...
# (run for once only!) read in and process top N-gene logic-gated CARs identified by LogiCAR Designer to standard format
```{r}
...
for (pp in patients_prefix){
  ...
  top_logiCARs_data_dir = paste0("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/", pp, "/")
...
# (run for once only! ) load and process sampled normal cell RNA expression data (the LogiCAR Designer input)
```{r}
...
####### load and process sampled tumor cell RNA expression data (the LogiCAR Designer input)
# server
data_dir = "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/02.Input/"
processed_dir <- data_dir
# local
data_dir = "/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/02.Input/"
processed_dir <- data_dir
...
LogiCAR_input_df[1:5,1:5]
class(LogiCAR_input_df)
...
dim(Bassez_CohortA_BIOKEY_10_Smat)
geneNAs_temp <- LogiCAR_input_df$V1[4:nrow(LogiCAR_input_df)]
Bassez_CohortA_BIOKEY_10_Smat[1:5,1:5]
...
dim(Bassez_CohortA_BIOKEY_10_final_matrix)
Bassez_CohortA_BIOKEY_10_final_matrix[1:5,1:5]
## convert - in gene names to _
...
dim(GSE161529_patient0135_Smat)
geneNAs_temp <- LogiCAR_input_df$V1[4:nrow(LogiCAR_input_df)]
GSE161529_patient0135_Smat[1:5,1:5]
...
dim(GSE161529_patient0135_final_matrix)
GSE161529_patient0135_final_matrix[1:5,1:5]
## convert - in gene names to _
...
dim(GSE246613_Patient11_Smat)
geneNAs_temp <- LogiCAR_input_df$V1[4:nrow(LogiCAR_input_df)]
GSE246613_Patient11_Smat[1:5,1:5]
...
dim(GSE246613_Patient11_final_matrix)
GSE246613_Patient11_final_matrix[1:5,1:5]
## convert - in gene names to _
...
save(filtered_top_logiCARs_list, file = "/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/Bassez_CohortA_BIOKEY_10_allTumorCells/filtered_top_logiCARs_v2.RData")


```

# load filtered_top_logiCARs_list
```{r}

load(file = "/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/Bassez_CohortA_BIOKEY_10_allTumorCells/filtered_top_logiCARs_v2.RData") # filtered_top_logiCARs_list
...
### old
# Computational_CAR_bulk = c("FAP & EPHA2_not", "FAP & MET_not", "OR2B6 & MET_not", "SLC1A4 & SSTR1_not", "ADAM12 & KDR_not", "EPHB3 & FZD5_not",  "ADAM12 & GPC3_not", "MUC1 & LYVE1_not", "SLC4A5 & PTH1R_not", "FLVCR1 & TRHDE_not", "EPHB3 & SLC16A7_not", "EPCAM & SSTR1_not", "MUC1 & SLC4A4_not", "TNFRSF18 & SLC16A7_not", "LDLRAD3 & SSTR1_not")
#### top 10
...
for (pp in names(filtered_top_logiCARs_list)){
  ...
  #Computational_CARs_df$Group = "Previous Computational"
...
# Figure 4ABC. barplot rank BRCA CAR targets based on  efficacy on each patient
```{r}
...
existing_logiCARs_with_safety = read.csv("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/filtered_top_logiCARs_efficacy_safety_17Datasets.csv")
...
dim(filtered_targets_result_df)
...
write.csv(filtered_targets_result_df, paste0("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/",name_for_data,"/filtered_targets_result_df_v2.csv"), row.names = F, quote = F)

### load from file
#filtered_targets_result_df = read.csv(paste0("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/",name_for_data,"/filtered_targets_result_df_v2.csv"))
...
##### do not compare with general LogiCAR
...
# Figure 4e. UMAP visualization of coverage of tumor cells by different CARs (in Dataset: GSE161529; Patient: 0135)
```{r}

### GSE161529 (patient: 0135)
dataset_name = 'GSE161529'
file_prefix = "seu_Tissue"
data_dir <- paste0("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/02.Input/", dataset_name, "/")
processed_dir <- data_dir
...
##### renormalization and UMAP based on surface genes
# seurat_obj_subset <- NormalizeData(seurat_obj_subset) # log(x/sum(x)*10000+1)
# seurat_obj_subset <- FindVariableFeatures(seurat_obj_subset)
# seurat_obj_subset <- ScaleData(seurat_obj_subset) # pbmc <- SCTransform(pbmc, vars.to.regress = "CC.Difference")
# seurat_obj_subset <- RunPCA(seurat_obj_subset, pc.genes = seurat_obj_subset@var.genes)
# seurat_obj_subset <- RunUMAP(seurat_obj_subset, reduction = "pca", dims = 1:30)

##### renormalization and UMAP based on all potential CAR genes
# gene_names <- unique(unlist(regmatches(filtered_targets_df_list[["Bassez_CohortA_BIOKEY_10_allTumorCells"]]$LogiCARs, gregexpr("\\b[A-Za-z0-9]+\\b", filtered_targets_df_list[["Bassez_CohortA_BIOKEY_10_allTumorCells"]]$LogiCARs))))
# gene_names <- c(gene_names, "ITM2B")
...
# seurat_obj_subset <- RunUMAP(seurat_obj_subset, dims = 1:5)  # Use the first 10 PCs
...
#seurat_obj_subset$CAR_MSLN <- (expression_data["MSLN", ] > 0)
...
#seurat_obj_subset$CAR_CD70 <- (expression_data["CD70", ] > 0)
seurat_obj_subset$CAR_NECTIN4 <- (expression_data["NECTIN4", ] > 0)
seurat_obj_subset$CAR_LDLRAD3 <- (expression_data["LDLRAD3", ] > 0)

# seurat_obj_subset$CAR_EMP2 <- (expression_data["EMP2", ] > 0)
# seurat_obj_subset$CAR_TACSTD2 <- (expression_data["TACSTD2", ] > 0)
# seurat_obj_subset$CAR_ITM2B_not <- (expression_data["ITM2B", ] == 0)
# seurat_obj_subset$CAR_LY6K_EMP2 <- (expression_data["LY6K", ] > 0 & expression_data["EMP2", ] > 0)
# seurat_obj_subset$CAR_ITM2B_not_EMP2 <- (expression_data["ITM2B", ] == 0 & expression_data["EMP2", ] > 0)
# seurat_obj_subset$CAR_TACSTD2_EMP2 <- (expression_data["TACSTD2", ] > 0 & expression_data["EMP2", ] > 0)
# seurat_obj_subset$CAR_LY6K_TACSTD2_EMP2 <- (seurat_obj_subset$CAR_LY6K_EMP2 | seurat_obj_subset$CAR_TACSTD2_EMP2)
#seurat_obj_subset$CAR_ITM2B_not_TACSTD2_EMP2 <- (seurat_obj_subset$CAR_ITM2B_not_EMP2 | seurat_obj_subset$CAR_TACSTD2_EMP2)
...
for (ct in CAR_targets){
  ...
  DimPlot(
    ...
    NoLegend()                      
  ggsave(
    filename = paste0(fig_dir, "UMAP_Bassez_CohortA_BIOKEY_10_tumor_",ct,".pdf"), # Change file extension to .pdf
    height = 90, 
    width = 90, 
    units = "mm", # Ensure units are in mm
    device = "pdf" # Specify PDF as the output device
