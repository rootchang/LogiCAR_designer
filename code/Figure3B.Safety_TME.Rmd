---
title: "Evaluate the safety of potential CARs in TME"
...
# (run only once!!) load and process expression data to sparse matrix with negated genes
```{r}
...
#### Save final_matrix_list
# saveRDS(final_matrix_list, "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/17datasets_tumor_vs_nonTumor_expr_final_matrix_list.RDS")
#### Save tumor_cellNum_vec
# saveRDS(tumor_cellNum_vec, "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/17datasets_tumor_cellNum_vec.RDS")
#### Save candidateCARs_equivalent_RNA_Expression_list
# saveRDS(candidateCARs_equivalent_RNA_Expression_list, "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/17datasets_tumor_vs_nonTumor_candidateCARs_equivalent_RNA_Expression_list.RDS")
...
#### Load final_matrix_list
# final_matrix_list <- readRDS("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/17datasets_tumor_vs_nonTumor_expr_final_matrix_list.RDS")
#### Load tumor_cellNum_vec
tumor_cellNum_vec <- readRDS("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/17datasets_tumor_cellNum_vec.RDS")
#### Load candidateCARs_equivalent_RNA_Expression_list
candidateCARs_equivalent_RNA_Expression_list <- readRDS("/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/safety90/17datasets_tumor_vs_nonTumor_candidateCARs_equivalent_RNA_Expression_list.RDS")
...
p <- ggplot(plot_data, aes(x = Gene, y = Dataset)) +  # Use Dataset for y-axis
  ...
  scale_fill_gradient2(
    ...
    mid = "white", 
    high = "red", 
    oob = scales::squish,  # Squish out-of-bounds values to the extreme colors
    midpoint = 0,  # Midpoint at fold change = 1
    limits = c(-2, 2)  # Set the range from 0 to 10
  ) +
  ...
  theme(
    ...
    legend.position   = c(1.08, 0.2), # "right"
    plot.margin = margin(r = 100)  # Add 50 units of space to the right margin
  )
...
# Sigure *. plot the dotplot betweeen tumor vs non-tumor cells
```{r}
...
for (fn in names(seu_list)) {
  ...
  if (Non_tumor_exist) {
    ...
    #------------------------------
    # 1) Compute fold change for each Gene: (Tumor / Non-tumor)
    #    - if both mean expressions = 0, define 0/0 = 1
    #    - cap any fold change > 5 at 5 (also covers Inf)
    #------------------------------
    ...
    # PDF settings
    pdf_file   <- file.path(fig_dir, paste0("Figure3b_safety_landscape_TME_", fn, ".pdf"))
    fig_width  <- 4
    fig_height <- 12.5
    fontSize   <- 1.2
    ...
    #------------------------------
    # 2) Plot:
    #    - First geom_point() for Tumor & Non-tumor
    #       (size = ExprRatio, color = MeanExpression)
    #    - Then use ggnewscale::new_scale("size") so we can have a second size legend for foldChange.
    #    - Second geom_point() for "FoldChange" squares
    #       (size = foldChange, shape=22, a separate legend)
    #------------------------------
    ...
    # Create the plot
    pdf_file <- paste0(fig_dir,paste0("Figure3b_safety_landscape_TME_",fn,".pdf"))
    fig_width = 3.6
    fig_height = 12.5
    fontSize = 1.2
    # Open PDF device
