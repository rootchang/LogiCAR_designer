...
sinteractive --mem=64g --cpus-per-task=16 --time=24:00:00
...
library(tibble)  # Load the tibble package for rownames_to_column()
...
# (run on server, for once only!) get the max personalized efficacy for each patient in Stefan cohort 
```{r}
...
for (pp in Stefan_patients) {
  ...
  result_dir_path_temp <- paste0(
    "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/01.Scripts/Triplet_CART_designer/output/", 
...
write.csv(result_df, file = "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/03.Results/Stefan_cohort_personalized_efficacy_66patients.csv", quote = FALSE, row.names = F)


```

# STable 3. top 50 personalized efficacy for each patient in Stefan cohort 
```{r}
...
write.csv(result_df, file = "/data/Lab_ruppin/tiangen/CancerProject/08.Sanna_CART/03.Results/Stefan_cohort_personalized_LogiCARs_and_Efficacy_66patients.csv", quote = FALSE, row.names = F)
...
max_scores_df <- read.csv("/Users/changt7/Documents/00.PostDocWork/CancerResearch/01.Projects/00.Collaborating_projects/08.Sanna_CART/03.Results/Stefan_cohort_personalized_efficacy_66patients.csv")
...
pdf(paste0(fig_dir, "PatientLevel_MaxEfficacy_barplot_Stefan.pdf"), width = 10, height = 4.2)
...
pdf(paste0(fig_dir, "Patient_Efficacy_Stefan_Personalized_vs_general_CARs_boxplot.pdf"), width = 3*0.8, height = 4*0.8)
